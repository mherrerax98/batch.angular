...
import { Component, NgModule, Input, OnInit, EventEmitter, Output } from '@angular/core';
...
export class LoginFormComponent implements OnInit {
  ...
  // @Output() onSignInChange: EventEmitter<boolean> = new EventEmitter<boolean>();
  ...
  async onSubmit(e: Event) {
    ...
    this.loading = false;
    if (!result.isOk) {
      // this.onSignInChange.emit(false);
      notify(result.message, 'error', 2000);
    }
    // else {
    //   this.onSignInChange.emit(true);
    // }
  }
